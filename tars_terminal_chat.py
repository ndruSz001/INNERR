    ...
    while True:
        ...
        guardar_trigger = any(
            frase in comando for frase in [
                "guarda esto", "guardar esto", "cambiar de tema", "nueva conversación", "nuevo tema", "save this", "change topic", "new conversation"]
        )
        ...
        if salir_trigger:
            if historial:
                _preguntar_guardar(historial, utiles_actuales, carpeta_guardado if modo_investigacion else None)
...
def _preguntar_guardar(historial, utiles_actuales):
    import datetime
    import os
    while True:
        ...
        if guardar == "s":
            ...
            ruta = input("Nombre de archivo (incluye carpeta si es para un agente): ").strip()
            if not ruta:
                ruta = sugerencia
            if hasattr(historial, 'carpeta_guardado') and historial.carpeta_guardado:
                ruta_final = os.path.join(historial.carpeta_guardado, ruta)
            elif isinstance(historial, list) and len(historial) > 0 and isinstance(historial[0], dict) and 'carpeta_guardado' in historial[0]:
                ruta_final = os.path.join(historial[0]['carpeta_guardado'], ruta)
            elif isinstance(historial, list) and hasattr(historial, 'carpeta_guardado'):
                ruta_final = os.path.join(historial.carpeta_guardado, ruta)
            elif isinstance(historial, list) and hasattr(historial, '__carpeta_guardado'):
                ruta_final = os.path.join(historial.__carpeta_guardado, ruta)
            elif isinstance(historial, list) and hasattr(historial, 'carpeta_guardado'):
                ruta_final = os.path.join(historial.carpeta_guardado, ruta)
            elif isinstance(historial, list) and hasattr(historial, 'carpeta_guardado'):
                ruta_final = os.path.join(historial.carpeta_guardado, ruta)
